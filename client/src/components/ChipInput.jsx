    ...
    <div className="flex flex-col space-y-2">
      <div className="flex w-full flex-wrap gap-y-2">
        <div className='flex justify-center items-center text-blue-400'>
          {
            `${label}` === "Tag People" ? <BsFillTagFill size={'30'}/> : <MdLocationPin size={'30'} />
          }
          <p className='text-lg'>{label}</p>
        </div>
        {
          `${label}` === "Tag People" ? <input
                                          id='tagPeople'
                                          type="text"
        ...
        
        {
          chips.map((chip, index) => (
            <div key={index} className="m-1 flex items-center rounded-full bg-gray-200 px-2 py-1 text-sm text-richblack-5">
              <button
                type="button"
